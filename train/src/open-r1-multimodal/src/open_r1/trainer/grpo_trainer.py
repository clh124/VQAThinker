...
class VLMGRPOTrainer(Trainer):
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        if self.temporal:
            ...
            rank_mean = temporal_rewards_per_func[:, 0].mean()
            shuffled_rank_mean = shuffled_rewards_per_func[:, 0].mean()

            reg_mean = temporal_rewards_per_func[:, 1].mean()
            shuffled_reg_mean = shuffled_rewards_per_func[:, 1].mean()

            if rank_mean >= shuffled_rank_mean:
                rank_mask = temporal_rewards_per_func[:, 0] > 0.5
                temporal_rewards_per_func[rank_mask, 0] += 0.3

            if reg_mean >= shuffled_reg_mean:
                reg_mask = temporal_rewards_per_func[:, 1] > 0.9
                temporal_rewards_per_func[reg_mask, 1] += 0.3
