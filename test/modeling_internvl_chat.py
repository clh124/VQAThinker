...
class slowfast(torch.nn.Module):
    ...
    def forward(self, x):
        with torch.no_grad():
            ...
            feature_3D = torch.cat([slow_feature, fast_feature],dim=1) 
...
class InternVLChatModel(PreTrainedModel):
    ...
    def forward(
        ...
        frames = pixel_values.view(B, int(vit_batch_size/B), 3, 560, 560)
        frames = frames.permute(0, 2, 1, 3, 4)
        device = pixel_values.device

        inputs = pack_pathway_output(frames, device) 
    ...
    def batch_chat(self, tokenizer, pixel_values, questions, generation_config, num_patches_list=None,
        ...
        for idx, num_patches in enumerate(num_patches_list):
            ...
            for num_patch in num_patches:
                image_tokens = IMG_START_TOKEN + IMG_CONTEXT_TOKEN * int(self.num_image_token * num_patch) + IMG_END_TOKEN
                query = query.replace('<image>', image_tokens, 1)
