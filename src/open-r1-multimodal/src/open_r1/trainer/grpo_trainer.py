...
class VLMGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print("model_cls",  model_cls) # <class 'transformers.models.auto.modeling_ auto.AutoModel'>
        print("model_init_kwargs", model_init_kwargs) # {'torch dtype': 'bfloat16','trust remote code': True, 'use flash attn': True}
        model = model_cls.from_pretrained(model_id, **model_init_kwargs)


        # # 保存模型权重和配置
        # save_path = "/dev/shm/InternVL3-8B-slowfast"  # 例如 "./saved_model"
        # model.save_pretrained(save_path)
        # print("sucessfully saved!")


        # non_lora_state_dict = torch.load("/dev/shm/internvl3_8B_lsvq_grpo/checkpoint-50/non_lora_state_dict.bin", map_location="cpu")
        # non_lora_state_dict = {
        #     k.replace("base_model.model.", "", 1).replace("base_layer.", "", 1): v
        #     for k, v in non_lora_state_dict.items()
        # }

        # # 更新模型参数（注意：只更新已存在 key）
        ...
        print("Freezing slowfast modules...")
        ...
        if is_peft_model(model):
            # print("model:", model)
            # print("model.base_model:", getattr(model, "base_model", None))
            # print("model.base_model.model:", getattr(getattr(model, "base_model", None), "model", None))
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        for x in inputs:
            if "image_path" in x and x["image_path"] is not None:
                imgs = [p for p in self._get_key_from_inputs(x, "image_path")]
            else:
                imgs = []
        ...
        temporal = True
        if temporal:
            shuffled_prompt_inputs = self.vlm_module.prepare_model_inputs(
                self.processing_class,
                prompts_text,
                images,
                return_tensors="pt",
        ...
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            ...
            if temporal:
                shuffled_generate_returned_result = unwrapped_model.generate(
                    **{k: v for k, v in shuffled_prompt_inputs.items() if k not in self.vlm_module.get_non_generate_params()}, 
                    generation_config=self.generation_config
                )
                shuffled_prompt_length = shuffled_prompt_ids.size(1)
                if not self.vlm_module.is_embeds_input():
                    shuffled_prompt_completion_ids = shuffled_generate_returned_result
                    ...
                    # In this case, the input of the LLM backbone is the embedding of the combination of the image and text prompt
                    # So the returned result of the `generate` method only contains the completion ids
                    shuffled_completion_ids = shuffled_generate_returned_result
                    shuffled_prompt_completion_ids = torch.cat([shuffled_prompt_ids, shuffled_completion_ids], dim=1)
        ...
        if temporal:
            ...
            shuffled_rewards_per_func = torch.zeros(len(prompts), len(self.reward_funcs), device=device)
            for i, (reward_func, reward_processing_class) in enumerate(
                zip(self.reward_funcs, self.reward_processing_classes)
            ):
                if isinstance(reward_func, PreTrainedModel):
                    if is_conversational(inputs[0]):
                        shuffled_messages = [{"messages": p + c} for p, c in zip(prompts, shuffled_completions)]
                        shuffled_texts = [apply_chat_template(x, reward_processing_class)["text"] for x in shuffled_messages]
                    else:
                        shuffled_texts = [p + c for p, c in zip(prompts, completions)]
                    shuffled_reward_inputs = reward_processing_class(
                        shuffled_texts, return_tensors="pt", padding=True, padding_side="right", add_special_tokens=False
                    )
                    shuffled_reward_inputs = super()._prepare_inputs(shuffled_reward_inputs)
                    with torch.inference_mode():
                        shuffled_rewards_per_func[:, i] = reward_func(**shuffled_reward_inputs).logits[:, 0]  # Shape (B*G,)
                else:
                    # Repeat all input columns (but "prompt" and "completion") to match the number of generations
                    ...
                    for key in shuffled_reward_kwargs:
                        for example in inputs:
                            # No need to duplicate prompts as we're not generating multiple completions per prompt
                            # reward_kwargs[key].extend([example[key]] * self.num_generations)
                            shuffled_reward_kwargs[key].extend([example[key]])

                    shuffled_reward_kwargs["num_generations"] = self.num_generations
                    shuffled_reward_kwargs["device"] = device
                    
                    shuffled_output_reward_func = reward_func(prompts=prompts, completions=shuffled_completions, **shuffled_reward_kwargs)
        ...
        #     print("shuffled_acc_mean", shuffled_acc_mean)
        #     print("raw:", temporal_rewards_per_func[:, 0])
        #     print("shuffled:", shuffled_rewards_per_func[:, 0])

        #     if acc_mean >= shuffled_acc_mean:
        #         print("acc win")
        #         # change true select
        #         mask = temporal_rewards_per_func[:, 0] > 0.5 # only true reward!
        ...
        if temporal:
            ...
            reg_mean = temporal_rewards_per_func[:, 2].mean()
            shuffled_reg_mean = shuffled_rewards_per_func[:, 2].mean()

            # print("acc_mean:", acc_mean)
            # print("shuffled_acc_mean:", shuffled_acc_mean)
            # print("reg_mean:", reg_mean)
            # print("shuffled_reg_mean:", shuffled_reg_mean)
            # print("raw acc:", temporal_rewards_per_func[:, 0])
            # print("shuffled acc:", shuffled_rewards_per_func[:, 0])
            # print("raw reg:", temporal_rewards_per_func[:, 2])
            # print("shuffled reg:", shuffled_rewards_per_func[:, 2])

            if acc_mean >= shuffled_acc_mean and reg_mean >= shuffled_reg_mean:
                # print("acc and reg win")
                # enhance true reward positions
                # acc 维度判断
                acc_mask = temporal_rewards_per_func[:, 0] > 0.5
                temporal_rewards_per_func[acc_mask, 0] += 0.3  # boost acc reward only

                # reg 维度判断（第三维度）
                # reg_mask = temporal_rewards_per_func[:, 2] > 0.5
                # temporal_rewards_per_func[reg_mask, 2] += 0.3  # boost reg reward only
            else:
                temporal_rewards = torch.tensor([0.0]).to('cuda')
        else:
            temporal_rewards = torch.tensor([0.5]).to('cuda')
        ...
        if temporal:
            ...
            rewards_per_func = self.accelerator.gather(temporal_rewards_per_func)
    ...
    def _save_checkpoint(self, model, trial):
        ...
        non_lora_weights = {
            ...
            if "lora_" not in k and "language" not in k
        }
        torch.save(non_lora_weights, os.path.join(output_dir, "non_lora_state_dict.bin"))
