...
# def regression_reward(content, sol, **kwargs):
#     print("content", content)
#     reward = 0.0
#         # Extract answer from solution if it has think/answer tags
#     sol_match = re.search(r'<answer>(.*?)</answer>', sol)
...
#     # Try symbolic verification first for numeric answers
#     try:
#         rel_diff = (abs(float(student_answer) - float(ground_truth)) + 1e-9) / (abs(float(ground_truth)) + 1e-9)
#         rel_diff = min(1.0, max(0.0, rel_diff))
#         reward = 1 - rel_diff
#     except Exception:
#         pass  # Continue to next verification method if this fails

#     print("regression reward", reward)
#     return reward
        ...
        for j in range(len(batch_pred[i])):
            ...
            for z in range(len(batch_mean)):
                if z != i:
                    ...
                    # print(input_pred1, input_pred2, input_gt)
...
def accuracy_reward2(completions, solution, **kwargs):
    ...
    for content, sol, accu_reward_method in zip(contents, solution, kwargs.get("accu_reward_method")):
        ...
        elif accu_reward_method == 'regression':
            reward = regression_reward(content, sol, **kwargs)
        else:
    ...
    for i in range(len(reshaped_content)):  # batch
        for j in range(len(reshaped_content[i])):  # num generations
            ...
            if os.getenv("DEBUG_MODE") == "true":
                ...
                with open(log_path, "a", encoding='utf-8') as f:
                    f.write(f"------------- {current_time} Diversity reward: {reward} -------------\n")
...
def main(script_args, training_args, model_args):
    ...
    script_args.reward_funcs = ['accuracy', 'format', "diversity"]
    ...
    script_args.reward_method = "regression"
        ...
        if 'image_path' in example and example['image_path'] is not None:
            ...
            return {
                ...
                'prompt': [{
                    ...
                    'content':
                    #  [
                    #     *({'type': 'image', 'text': None} for _ in range(8)),
                    #     {'type': 'text', 'text': question_prompt.format(Question=example['problem'])}
                    # ]
                    [
                        {'type': 'text', 'text': 'Now you will receive a video:\n'},
                    ] + [
                        elem
                        for i in range(6)
                        for elem in (
    ...
    dataset = dataset.map(make_conversation_from_jsonl, num_proc=100)
